...
{
	public class CharacterModding : FrostyModule
	{
		...
		public List<NetTexture> netTextures = new List<NetTexture>();
		/// <summary>
		...
		*/
        ...
        public override void Show()
		{
			...
			if (GUILayout.Button("Save rider"))
			{
                ...
                if (MultiplayerManager.isConnected() && LocalPlayer.instance.RiderModelname == "Daryien")
                {
				   RipTideOutgoing.SendGearUpdate(MultiplayerManager.instance.GetMyGear(true));
                }
			}
			GUILayout.Space(2);
		...
		{
			if (LocalPlayer.instance.RiderParentobj.GetComponentsInChildren<Transform>().Length > 70)
            {
				FrostyPGamemanager.instance.PopUpMessage(("Rider setup only available for Daryien"));
				return;
            }
		...
		public void SaveRiderSetup()
        {
			...
			string json = TinyJson.JSONWriter.ToJson(data);
			File.WriteAllText(MultiplayerManager.Playersavedir + "RiderSave.json", json);
		...
		public int LoadRiderSetup()
        {
			...
			if (File.Exists(MultiplayerManager.Playersavedir + "RiderSave.json"))
            {
				load = TinyJson.JSONParser.FromJson<RiderSaveData>(File.ReadAllText(MultiplayerManager.Playersavedir + "RiderSave.json"));
		...
		public List<NetTexture> GetNetTextures()
        {
			List<NetTexture> list = new List<NetTexture>();
			foreach (Renderer r in Rider_Materials.Values)
			{
				for (int i = 0; i < r.materials.Length; i++)
				{
					if (r.materials[i].mainTexture != null)
					{
						...
						if (bytes != null)
						{
							...
							string texturename = r.materials[i].mainTexture.name;
							// find actual file

							foreach (FileInfo file in new DirectoryInfo(MultiplayerManager.TexturesDir).GetFiles("*.*", SearchOption.AllDirectories))
							{
								if (file.Name == r.materials[i].mainTexture.name)
								{
									list.Add(new NetTexture(texturename, r.gameObject.name, false, i, file.DirectoryName));
						...
						{
							list.Add(new NetTexture("stock", r.gameObject.name, false, i, "none"));
						}

					}
					else
					{
						list.Add(new NetTexture("e", r.gameObject.name, false, i, "none"));
					}
