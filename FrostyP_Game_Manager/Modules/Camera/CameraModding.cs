...
{
	public class CameraModding : FrostyModule
	{
		public static CameraModding instance;
		...
		string PresetDirectory = Application.dataPath + "/FrostyPGameManager/Presets/Camera/";
		...
		public override void Show()
		{
			...
			if(Camera.current.gameObject != CharacterModding.instance.Cam)
            {
			...
			if (GUILayout.Button("Close Camera Setup"))
			{
                ...
                {
				InGameUI.instance.ShowCamSettingsOverride = false;
				}
                else
                {
			...
			GUILayout.Space(10);
			GUILayout.Label(lastpresetselectedCAM + " Profile active", InGameUI.instance.Generalstyle);
			GUILayout.Space(20);
			...
			GUILayout.Label("Camera FOV = " + CamFOVCAMSETTING , InGameUI.instance.MiniPanelStyle);
			CamFOVCAMSETTING = GUILayout.HorizontalSlider(CamFOVCAMSETTING, 0, 100);
			GUILayout.Label("Bloom Intensity = " + newbloomintvalueCAMSETTING, InGameUI.instance.MiniPanelStyle);
			newbloomintvalueCAMSETTING = GUILayout.HorizontalSlider(newbloomintvalueCAMSETTING, 0, 1f);
			GUILayout.Label("Bloom Threshold = " + newbloomvalueCAMSETTING, InGameUI.instance.MiniPanelStyle);
			newbloomvalueCAMSETTING = GUILayout.HorizontalSlider(newbloomvalueCAMSETTING, 0, 10);
			GUILayout.Label("Aperture (0-16) = " + apertureCAMSETTING, InGameUI.instance.MiniPanelStyle);
			apertureCAMSETTING = GUILayout.HorizontalSlider(apertureCAMSETTING, 0, 16);
			GUILayout.Label("Focus Distance (0-10) = " + focusdistanceCAMSETTING, InGameUI.instance.MiniPanelStyle);
			focusdistanceCAMSETTING = GUILayout.HorizontalSlider(focusdistanceCAMSETTING, 0, 10);
			GUILayout.Label("Contrast (0-75) = " + ContrastNewCAMSETTING, InGameUI.instance.MiniPanelStyle);
			ContrastNewCAMSETTING = GUILayout.HorizontalSlider(ContrastNewCAMSETTING, 0, 75);
			GUILayout.Label("Saturation (-100 - 50) = " + SaturationNewCAMSETTING, InGameUI.instance.MiniPanelStyle);
			SaturationNewCAMSETTING = GUILayout.HorizontalSlider(SaturationNewCAMSETTING, -100, 50);
			GUILayout.Label("Brightness (-50 - 50) = " + BrightnessNewCAMSETTING, InGameUI.instance.MiniPanelStyle);
			BrightnessNewCAMSETTING = GUILayout.HorizontalSlider(BrightnessNewCAMSETTING, -50, 50);
			GUILayout.Label("Temperature (-30 - 30) = " + temperatureNewCAMSETTING, InGameUI.instance.MiniPanelStyle);
			temperatureNewCAMSETTING = GUILayout.HorizontalSlider(temperatureNewCAMSETTING, -30, 30);

			GUILayout.Label("Vignette Intensity (0 - 1) = " + vignetteintensityCAMSETTING, InGameUI.instance.MiniPanelStyle);
			vignetteintensityCAMSETTING = GUILayout.HorizontalSlider(vignetteintensityCAMSETTING, 0, 1f);

			GUILayout.Label("Vignette Roundness (0 - 1) = " + VignetteRoundnessCAMSETTING, InGameUI.instance.MiniPanelStyle);
			VignetteRoundnessCAMSETTING = GUILayout.HorizontalSlider(VignetteRoundnessCAMSETTING, 0, 1);

			GUILayout.Label("Grain Intensity (0 - 1) = " + GrainIntensityCAMSETTING, InGameUI.instance.MiniPanelStyle);
			GrainIntensityCAMSETTING = GUILayout.HorizontalSlider(GrainIntensityCAMSETTING, 0, 1);

			GUILayout.Label("Grain Light contribution (0 - 1) = " + GrainLightcontribCAMSETTING, InGameUI.instance.MiniPanelStyle);
			GrainLightcontribCAMSETTING = GUILayout.HorizontalSlider(GrainLightcontribCAMSETTING, 0, 1);

			GUILayout.Label("Grain Size (0 - 1) = " + GrainSizeCAMSETTING, InGameUI.instance.MiniPanelStyle);
			GrainSizeCAMSETTING = GUILayout.HorizontalSlider(GrainSizeCAMSETTING, 0, 1);

			GUILayout.Label("Hue Shift (-50 - 50) = " + HueShiftCAMSETTING, InGameUI.instance.MiniPanelStyle);
			...
			GUILayout.Space(50);
				...
				if (GUILayout.Button("Save as"))
				{
					...
					List<CameraSetting> Camsavelist = new List<CameraSetting>();
					...
					foreach (MemberInfo info in members)
					{
						...
						{
							FieldInfo field = typeof(CameraModding).GetField(info.Name);
							...
							if (field.FieldType.ToString().Contains("Single") | field.FieldType.ToString().Contains("Int"))
							{
								if (info.Name.Contains("CAMSETTING"))
								{
									...
									if (field.FieldType.ToString().Contains("Single"))
									{
										...
										Camsavelist.Add(new CameraSetting(info.Name, obj));
					...
					SaveLoad.Save(new CameraPreset(name,Camsavelist), PresetDirectory);
				...
				foreach (FileInfo file in camfilestoload)
				{
					...
					if (file.Name.Contains(".json"))
					{
						...
						{
							MemberInfo[] members = this.GetType().GetMembers();
							CameraPreset Preset = SaveLoad.Load(file.FullName);
							...
							{
								foreach (CameraSetting setting in Preset.settings)
								{
									if (member.Name.CompareTo(setting.nameofsetting) == 0)
									{
										FieldInfo field = typeof(CameraModding).GetField(member.Name);
										if (field.FieldType.ToString().Contains("Single"))
										{



											float value = setting.valuefloat;
