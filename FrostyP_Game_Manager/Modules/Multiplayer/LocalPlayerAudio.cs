...
namespace FrostyP_Game_Manager
{
    ...
    public class LocalPlayerAudio : MonoBehaviour
    {
        ...
        void Update()
        {
            if (MultiplayerManager.isConnected() && Mylocalplayer.SendStream)
            {
                    ...
                    if (OneShotter.CurrentEvent.isValid())
                    {
                        ...
                        if (state == FMOD.Studio.PLAYBACK_STATE.STARTING | state == FMOD.Studio.PLAYBACK_STATE.PLAYING)
                        {
                            ...
                            OneShotter.CurrentEvent.getVolume(out float volume, out float finalvol);
                                ...
                                {
                                 OneShotUpdates.Add(new AudioStateUpdate(finalvol, ThisSoundpath,RiptideNetworking.MessageSendMode.unreliable));
                ...
                // take care of all FMODbyRiserVel's 
                ...
                {
                   RipTideOutgoing.SendAudioUpdate(OneShotUpdates, 2);
                ...
                if (RisersStateUpdate.Count > 0)
                {
                    ...
                    RipTideOutgoing.SendAudioUpdate(RisersStateUpdate, 1);
        ...
        void Playing(int state, FmodRiserByVel riser)
        {
            ...
            AudioStateUpdate update = new AudioStateUpdate(finalvol, finalpitch, state, soundname, _Vel, RiptideNetworking.MessageSendMode.unreliable);
        ...
        void Stopping(int state, FmodRiserByVel riser)
        {
            ...
            riser.sound.getPitch(out float pitch, out float finalpitch);
            string soundname = riser.gameObject.name;
            AudioStateUpdate update = new AudioStateUpdate(finalvol, finalpitch, state, soundname, 0, RiptideNetworking.MessageSendMode.reliable);
            RisersStateUpdate.Add(update);
        ...
        void Stopped(int state, FmodRiserByVel riser)
        {
            ...
            AudioStateUpdate update = new AudioStateUpdate(0, 0, state, riser.gameObject.name, 0, RiptideNetworking.MessageSendMode.reliable);
            RisersStateUpdate.Add(update);
        }

        void Sustaining(int state, FmodRiserByVel riser)
        {
            AudioStateUpdate update = new AudioStateUpdate(0, 0, state, riser.gameObject.name, 0, RiptideNetworking.MessageSendMode.unreliable);
            RisersStateUpdate.Add(update);
        ...
        void Starting(int state, FmodRiserByVel riser)
        {
            ...
            Vel.getValue(out float _Vel);
            string soundname = riser.gameObject.name;
        AudioStateUpdate update = new AudioStateUpdate(finalvol, finalpitch, state, soundname, _Vel, RiptideNetworking.MessageSendMode.reliable) ;
            RisersStateUpdate.Add(update);
