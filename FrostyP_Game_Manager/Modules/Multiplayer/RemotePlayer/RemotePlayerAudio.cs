...
namespace FrostyP_Game_Manager
{
    public class RemotePlayerAudio : MonoBehaviour
    {
        ...
        public Queue<AudioStateUpdate> IncomingRiserUpdates;
        public Queue<AudioStateUpdate> IncomingOneShotUpdates;
        ...
        void Awake()
        {
            IncomingRiserUpdates = new Queue<AudioStateUpdate>();
            IncomingOneShotUpdates = new Queue<AudioStateUpdate>();
        ...
        void Update()
        {
            ...
            if(IncomingRiserUpdates.Count > 0)
            {
                try
                {
                    ...
                    while (IncomingRiserUpdates.Count > 0)
                    {
                        AudioStateUpdate update = IncomingRiserUpdates.Dequeue();
                        StateHandlers[update.nameofriser]?.Invoke(update);
                        update.Release();
                    }
                     
                    while(IncomingOneShotUpdates.Count > 0)
                    {
                        AudioStateUpdate update = IncomingOneShotUpdates.Dequeue();
                        FMOD.Studio.EventInstance instance = FMODUnity.RuntimeManager.CreateInstance(update.Path);
                        instance.setVolume(Mathf.Clamp(update.Volume / (Vector3.Distance(Rider.transform.position, Camera.current.transform.position) * 0.5f),0,10));
        ...
        public void Shutdown()
        {
            ...
            while(IncomingOneShotUpdates.Count > 0)
            {
                IncomingOneShotUpdates.Dequeue().Release();
            }
            while(IncomingRiserUpdates.Count > 0)
            {
                IncomingRiserUpdates.Dequeue().Release();
            }
