...
namespace FrostyP_Game_Manager
{
    ...
    public class ParkBuilder : FrostyModule
    {
		...
		void Awake()
        {
			...
			AxisOBJ = new GameObject();
			DontDestroyOnLoad(AxisOBJ);
            for (int i = 0; i < 3; i++)
            {
				GameObject g = new GameObject();
				g.transform.parent = AxisOBJ.transform;
				g.AddComponent<LineRenderer>().material = new Material(Shader.Find("Specular"));
		...
		void FixedUpdate()
        {
            if (StreamingObjectData && MultiplayerManager.isConnected())
            {
                try
                {
					if(Activeobj != null && ActivePlacedObject != null)
                    {
				      ...
				      RipTideOutgoing.ObjectTransformUpdate(ActivePlacedObject);
			  ...
			  if (isOpen)
              {
				...
				if (GUILayout.Button("Clear All ", FrostyPGamemanager.instance.style) && placedobjects.Count>0)
				{
                    ...
                    for (int i = 0; i < placedobjects.Count; i++)
                    {
                        ...
                        {
						RipTideOutgoing.DestroyAnObject(placedobjects[i].ObjectId);
                        }
					...
					GUILayout.Space(2);
					snappingtofloor = GUILayout.Toggle(snappingtofloor, "Snap to floor", FrostyPGamemanager.instance.style);
					GUILayout.Space(2);
        ...
        {
			Vector3 mypos = LocalPlayer.instance.DaryienOriginal.transform.position;
			MultiplayerManager.TogglePlayerComponents(false);
			Activeobj = GameObject.CreatePrimitive(PrimitiveType.Sphere);
			Activeobj.name = "Pointer";
			Activeobj.transform.position = mypos + Vector3.up;
			...
			if (inputman == null)
			{
				...
				camobj.transform.position = mypos + (Vector3.back * 5) + (Vector3.up * 2);
		...
		void DestroyObject()
        {
			...
			if(ActivePlacedObject.ObjectId != 0 && MultiplayerManager.isConnected())
            {

				RipTideOutgoing.DestroyAnObject(ActivePlacedObject.ObjectId);
		...
		void PlaceObject()
        {
                  ...
                  if (!REgrabbed)
                  {
				    ...
				    PlacedObject _new = new PlacedObject(obj, ActiveBundleData);
                     ...
                     try
                     {
                       ...
                       if (MultiplayerManager.isConnected() && NetgameObjects.Count < 30)
                       {
								RipTideOutgoing.SpawnObjectOnServer(_newobj);
			           }
                     ...
                     {
				      Debug.Log("Object Register error: " + x);
                     }
            ...
            {
				Debug.Log("PlaceObject error : " + x + " : " + x.TargetSite);
            }
		...
		void MoveObject()
        {
            ...
            try
            {
				...
				if (snappingtofloor)
                {
			     floornorm = SnappingtoFloor();
                }
                else
                {
					floornorm = new Vector3();
                }
			  ...
			  if(MGInputManager.LStickY() > 0.2f | MGInputManager.LStickY() < -0.2f)
              {
			   ...
			   Vector3 removeY = new Vector3(camobj.transform.forward.x, floornorm.y, camobj.transform.forward.z);
			   if(Y!=0 | X != 0)
               {
			    Activeobj.transform.position = Activeobj.transform.position + (removeY * Y) + (camobj.transform.right * X);
               }
		...
		void ShowAxisofObject()
        {
            ...
            try
            {
			Collider coll = Activeobj.GetComponent<Collider>();
			LineRenderer[] lines = AxisOBJ.GetComponentsInChildren<LineRenderer>(true);
			LineRenderer line_Y = lines[0];
			line_Y.material.color = Color.red;
			line_Y.positionCount = 2;
			line_Y.startWidth = 0.01f;
			line_Y.endWidth = 0.1f;
			line_Y.SetPosition(0,Activeobj.transform.position + (Activeobj.transform.up * 1));
			line_Y.SetPosition(1, Activeobj.transform.position + (Activeobj.transform.up * 2));

			LineRenderer line_X = lines[1];
			line_X.material.color = Color.green;
			line_X.positionCount = 2;
			line_X.startWidth = 0.01f;
			line_X.endWidth = 0.1f;
			line_X.SetPosition(0, Activeobj.transform.position + (Activeobj.transform.right * 1));
			line_X.SetPosition(1, Activeobj.transform.position + (Activeobj.transform.right * 2));

			LineRenderer line_Z = lines[2];
			line_Z.material.color = Color.blue;
			line_Z.positionCount = 2;
			line_Z.startWidth = 0.01f;
			line_Z.endWidth = 0.1f;
			line_Z.SetPosition(0, Activeobj.transform.position + (Activeobj.transform.forward * 1));
			line_Z.SetPosition(1, Activeobj.transform.position + (Activeobj.transform.forward * 2));


            }
            catch (System.Exception x)
            {
				Debug.Log($"ShowAxis error: {x}");
            }
		...
		Vector3 SnappingtoFloor()
        {

           try
           {

			 Collider coll = Activeobj.GetComponentInChildren<Collider>();
				if(coll == null)
                {
					Debug.Log("no renderer found on active obj");
					return new Vector3();
                }
             ...
             if (Physics.SphereCast(ray,0.2f,out rayhit))
             {
				Activeobj.transform.position = new Vector3(Activeobj.transform.position.x,rayhit.point.y + size.y, Activeobj.transform.position.z);
        ...
        #region Spawning and loading objects
				...
				if (GUILayout.Button("Save Spot"))
                {
					foreach(PlacedObject _p in ObjectstoSave)
                    {
						...
						bool gotname = false;
                            ...
                            {
								if(_s == MultiplayerManager.Players[_p.OwnerID].username)
                                {
									gotname = true;
                                }
                            }
                            ...
                            if (!gotname)
                            {
								CreatorsList.Add(MultiplayerManager.Players[_p.OwnerID].username);
                            }
		...
		public void SceneSetupShow()
        {
			...
			if(lights != null && ShowingLight == null)
            {
				...
				if(GUILayout.Button($"{light.name} : {light.type.ToString()}",buttonsstyle))
                {
                        ...
                        {
						  lightX = light.gameObject.transform.eulerAngles.x;
						  lightY = light.gameObject.transform.eulerAngles.y;
						  lightZ = light.gameObject.transform.eulerAngles.z;
		...
		void DirectionalLightRotatePanel(Light light)
        {
			...
			GUILayout.BeginHorizontal();
			GUILayout.Label("X:");
			lightX = GUILayout.HorizontalSlider(lightX, 0, 360);
			GUILayout.EndHorizontal();

			GUILayout.BeginHorizontal();
			GUILayout.Label("Y:");
			lightY = GUILayout.HorizontalSlider(lightY, 0, 360);
			GUILayout.EndHorizontal();


			GUILayout.BeginHorizontal();
			GUILayout.Label("Z:");
			lightZ = GUILayout.HorizontalSlider(lightZ, 0, 360);
			GUILayout.EndHorizontal();
            ...
            if (!found)
            {
			light.gameObject.transform.eulerAngles = new Vector3(lightX, lightY, lightZ);
		...
		void SetSkyboxTexture(FileInfo file)
        {
			...
			Material mat = MultiplayerManager.PanoMat;
