...
namespace FrostyP_Game_Manager
{
    public class InGameUI : MonoBehaviour
    {
        ...
        /// <summary>
        /// populated by Spectate mode for cycling through players
        /// </summary>
        List<ushort> RiderIDs = new List<ushort>();
        ...
        public ushort IdofRidertoshow = 0;
        ...
        Dictionary<FrostyUIColor, Color> MessageColour;
        ...
        public List<ushort> ToggledOffPlayersByMe = new List<ushort>();

        float movespeed;
        float rotatespeed;

        public Stopwatch GethubDataWatch = new Stopwatch();
        public List<PublicServer> Publicservers = new List<PublicServer>();
        public bool UpdatingServerlist;
        public bool RequestingConnection;
        Vector2 publicserverscroll;
        Vector2 savedserverscroll;
        public bool Publicserverdetailwindowopen;
        public PublicServer Showingserver;
        ...
        {
            SetupGuis();
            MessageColour = new Dictionary<FrostyUIColor, Color>()
            {
                {FrostyUIColor.Me,Color.blue},
                {FrostyUIColor.Player,Color.black},
                {FrostyUIColor.System,Color.green},
                {FrostyUIColor.Server,Color.red},
            };
            ...
            if (!Directory.Exists(MultiplayerManager.Playersavedir))
            {
                Directory.CreateDirectory(MultiplayerManager.Playersavedir);
            }
            if (!File.Exists(MultiplayerManager.Playersavedir + "PlayerData.json"))
            {
                MultiplayerManager.PlayerSavedata = new PlayerSaveData(Username);
                MultiplayerManager.PlayerSavedata.savedservers = new List<SavedServer>();
                string json = TinyJson.JSONWriter.ToJson(MultiplayerManager.PlayerSavedata);
                File.WriteAllText(MultiplayerManager.Playersavedir + "PlayerData.json",json);

            }
            else if (File.Exists(MultiplayerManager.Playersavedir + "PlayerData.json"))
            {
                MultiplayerManager.PlayerSavedata = TinyJson.JSONParser.FromJson<PlayerSaveData>(File.ReadAllText(MultiplayerManager.Playersavedir + "PlayerData.json"));
                Username = MultiplayerManager.PlayerSavedata.Username;
            }
        ...
        void Update()
        {
            if (IsSpectating)
            {
                ...
                if (MGInputManager.RB_Down())
                {
                    ...
                    if (SelectedRiderID == RiderIDs.Count - 1)
                    {
                        SelectedRiderID = 0;
                    }
                    else
                    ...
                    if (MultiplayerManager.Players.ContainsKey(RiderIDs[SelectedRiderID]))
                    {
                        Targetrider = MultiplayerManager.Players[RiderIDs[SelectedRiderID]].RiderModel;
                ...
                if (MGInputManager.LB_Down())
                {
                    ...
                    {
                        SelectedRiderID = RiderIDs.Count - 1;
                    }
                    else
                    {
                        SelectedRiderID--;
            ...
            if (MultiplayerManager.isConnected() && Input.GetKeyDown(KeyCode.F12))
            {
                AdminOpen = !AdminOpen;
            }
            if (Input.GetKeyDown(KeyCode.Return) && MultiplayerManager.isConnected() && !Minigui)
            {
                if (Messagetosend.Length > 0)
                {
                    RipTideOutgoing.SendTextMessage(Messagetosend.ToString());
                    Messagetosend = "";
                }
            }
        ...
        void FixedUpdate()
        {
            if (IsSpectating && MultiplayerManager.isConnected())
            {
            CamModes[cyclemodes]();
            ...
            if (!MultiplayerManager.isConnected())
            {
                ...
                {
                    if (GethubDataWatch.IsRunning)
                    {
                        GethubDataWatch.Reset();
                    }
                }
            }
        }
        ...
        public void Show()
        {
            ...
            if (!MultiplayerManager.isConnected())
            {
                ClientsOfflineMenu();
                ShowPublicServers();
                DetailedServerWindowShow();
            }
            else if (MultiplayerManager.isConnected() && !IsSpectating)
            {
                ClientsOnlineMenu();
                LiveRiders();
                MessagesShow();
                if (RiderInfoMenuOpen) ShowRiderInfo();
                if (SyncWindowOpen) ShowSyncWindow();
                if (PlayeroptionsOpen) ShowPlayerOptions();
            ...
            else if (UpdateOpen) ShowUpdateWindow();
            else if (MultiplayerManager.isConnected() && IsSpectating)
            {
                if (SpectateShowGUI)
                {
                    ShowSpectate();
                    if (ShowCamSettingsOverride) CameraModding.instance.Show();
        ...
        {
            if(RiderIDs[SelectedRiderID] == id)
            {
                SwitchSpectateTarget();
            }
        }

        void SwitchSpectateTarget(bool up = true)
        {
            if (RiderIDs.Count < 2) SpectateExit();
            RiderIDs.RemoveAt(SelectedRiderID);
            System.Random random = new System.Random();
            SelectedRiderID = RiderIDs[random.Next(0, RiderIDs.Count - 1)];
            if(MultiplayerManager.Players.TryGetValue(RiderIDs[SelectedRiderID],out RemotePlayer player))
            {
                Targetrider = player.RiderModel;
                ControlObj.transform.position = player.RiderModel.transform.position + (Vector3.back * 2) + (Vector3.up);
                SpecCamOBJ.transform.position = player.RiderModel.transform.position + (Vector3.back * 2) + (Vector3.up);
        ...
        {
            GUI.skin = skin;
            GUILayout.BeginArea(new Rect(new Vector2(10, (Screen.height - (Screen.height / 1.2f)) / 2), new Vector2(Screen.width/6,Screen.height/1.2f)),BoxStyle);
            GUILayout.Space(10);
            ...
            {
               MultiplayerManager.ConnectMaster(desiredIP,desiredport);
            }
            GUILayout.Space(5);
            GUILayout.EndHorizontal();
                ...
                if(GUILayout.Button($"Save current setup as"))
                {
                    ...
                    if(MultiplayerManager.PlayerSavedata.savedservers == null)
                    {
                     MultiplayerManager.PlayerSavedata.savedservers = new List<SavedServer>();
                    }

                    if(MultiplayerManager.PlayerSavedata != null)
                    {
                     MultiplayerManager.PlayerSavedata.savedservers.Add(new SavedServer(desiredIP, desiredport, Nickname));
                    string json = TinyJson.JSONWriter.ToJson(MultiplayerManager.PlayerSavedata);
                    File.WriteAllText(MultiplayerManager.Playersavedir + "PlayerData.json", json);
            ...
            GUILayout.Space(10);
                if (MultiplayerManager.PlayerSavedata != null && MultiplayerManager.PlayerSavedata.savedservers != null)
                {
                ...
                removestyle.hover.textColor = Color.white;
                 ...
                 savedserversscroll = GUILayout.BeginScrollView(savedserversscroll);
                    foreach(SavedServer s in MultiplayerManager.PlayerSavedata.savedservers.ToArray())
                    {
                        GUILayout.BeginHorizontal();
                        ...
                        {
                          MultiplayerManager.PlayerSavedata.savedservers.Remove(s);
                        string json = TinyJson.JSONWriter.ToJson(MultiplayerManager.PlayerSavedata);
                        File.WriteAllText(MultiplayerManager.Playersavedir + "PlayerData.json", json);
        ...
        public void ClientsOnlineMenu()
        {
            ...
            if (MultiplayerManager.GarageEnabled)
            {
                ...
                {
                    RipTideOutgoing.SendGearUpdate(MultiplayerManager.instance.GetMyGear(false));
                }
            ...
            if (AdminOpen)
            {
                ...
                if (!AdminLoggedin)
                {
                    ...
                    {
                        RipTideOutgoing.AdminModeOn(Adminpass);
                    }
                ...
                if (AdminLoggedin)
                {
                    ...
                    {
                        RipTideOutgoing.AdminAlterBanWords(true, Banword);
                    }
                    if (GUILayout.Button("remove word"))
                    {
                        RipTideOutgoing.AdminAlterBanWords(false, Banword);
                    }
            ...
            if(bootplayeropen)
            {
                ...
                foreach (RemotePlayer r in MultiplayerManager.Players.Values)
                {
                    ...
                    {
                        RipTideOutgoing.SendBootPlayer(r.username, _bantime);
                    }
                }
                GUILayout.EndScrollView();
                GUILayout.EndArea();
                ...
                foreach (RemotePlayer r in MultiplayerManager.Players.Values)
                {
                    ...
                    if(r.Objects[i]._Gameobject != null)
                    {
                        ...
                        GUILayout.Label($"{r.username} objects:");
                            ...
                            {
                        RipTideOutgoing.AdminRemoveObject(r.id, r.Objects[i].ObjectID);
                            }
        ...
        {
            RiderIDs = MultiplayerManager.PlayerIds;
            if (RiderIDs.Count <= 0) return;
            
            // get initial target rider, check riders arent null
            bool gotInitialTarget = false;
            for (int i = 0; i < RiderIDs.ToArray().Length; i++)
            {
                if(MultiplayerManager.Players.TryGetValue(RiderIDs[i],out RemotePlayer player))
                {
                    if (!player.PlayerIsVisible) continue;
        ...
        public void SpectateTripodFullManual()
        {
            ...
            if (Targetrider == null)
            {
                Targetrider = MultiplayerManager.Players[RiderIDs[(int)UnityEngine.Random.Range(0,RiderIDs.Count-1)]].RiderModel;
        ...
        public void ShowSpectate()
        {
            ...
            GUILayout.Space(10);
            GUILayout.Label($"player:{MultiplayerManager.Players[RiderIDs[SelectedRiderID]].username} ");
            GUILayout.Space(10);
       ...
       public void MiniGUI()
        {
            ...
            GUILayout.Label($"People Riding: {MultiplayerManager.Players.Count}",MiniPanelStyle);
            GUILayout.Label($"On this map: {MultiplayerManager.instance.RidersOnMyMap()}",MiniPanelStyle);
            GUILayout.Label($"Ping: {MultiplayerManager.Ping / 1000} secs",MiniPanelStyle);
            GUILayout.Label($"Average delay: {MultiplayerManager.instance.GetAveragePing() / 1000} secs", MiniPanelStyle);
            ...
            {
                // from a player
                if(mess.uicolour == FrostyUIColor.Player)
                {
                    try
                    {
                        if(MultiplayerManager.Players.TryGetValue((ushort)mess.Mode,out RemotePlayer player))
                        {
                ...
                else
                {
                    ...
                    GUIStyle style = new GUIStyle();
                    style.normal.textColor = MessageColour[mess.uicolour];
                    style.alignment = TextAnchor.MiddleCenter;
        ...
        public void ShowRiderInfo()
        {
            ...
            if(IdofRidertoshow != 0)
            {
                try
                {
                    ...
                    if(MultiplayerManager.Players.TryGetValue(IdofRidertoshow,out RemotePlayer player))
                    {
                        ...
                        GUILayout.Label($"Net FPS: {Mathf.RoundToInt(player.FPS)}",Generalstyle);
                        ...
                        if (!MultiplayerManager.instance.RiderOnMyMap(player))
                        {
                            CurrentOverrideStyle = player.OverrideMapSync ? ButtonOnstyle : PlayeroptionsStyle;
                            if(GUILayout.Button($"Map sync Override {player.OverrideMapSync}",CurrentOverrideStyle))
                            {
                                player.OverrideMapSync = !player.OverrideMapSync;
                                RipTideOutgoing.OverrideAMapMatch(player.id, player.OverrideMapSync);
                            }
                        GUILayout.Space(10);
                        }
                        ...
                        if (GUILayout.Button("Invite to spawn",Generalstyle))
                        {
                            if(!MultiplayerManager.instance.RiderOnMyMap(player))
                            {
                               instance.NewMessage(Constants.ServerMessageTime, new TextMessage("Player not on your map", FrostyUIColor.Server, TextMessage.Textmessagemode.system));
                            }
                            else
                            {
                                SessionMarker marker = FindObjectOfType<SessionMarker>();
                                RipTideOutgoing.InviteToSpawn(player.id, marker.marker.position, marker.marker.eulerAngles);
                                InGameUI.instance.NewMessage(Constants.SystemMessageTime, new TextMessage($"Sent spawn to {player.username}", FrostyUIColor.System, TextMessage.Textmessagemode.system));
                            }
                        }
                        GUILayout.Space(10);
                        if (GUILayout.Button("Jump to Map",Generalstyle))
                        {
                            MultiplayerManager.instance.JumpToPlayerMap(player.CurrentMap);
                        }
                        ...
                        if (player.Objects.Count > 0)
                        {
                            ...
                            foreach (NetGameObject n in player.Objects)
                            {
                                ...
                                {
                                        RipTideOutgoing.VoteToRemoveObject(n.ObjectID, IdofRidertoshow);
                                }
        ...
        public void LiveRiders()
        {
            try
            {
                ...
                if (LiveRiderToggle)
                {
                    ...
                    float averageping = MultiplayerManager.instance.GetAveragePing();
                    string mostpop = "none";
                    ...
                    GUILayout.Space(20);
                    GUILayout.BeginHorizontal();
                    GUILayout.Label($"{MultiplayerManager.Players.Count} ",Contentright, GUILayout.MaxWidth(30));
                   ...
                   {
                        MultiplayerManager.instance.JumpToPlayerMap(mostpop);
                   }
        ...
        public void MessagesShow()
        {
            ...
            foreach (TextMessage mess in Messages)
            {
                ...
                if (mess.Mode == TextMessage.Textmessagemode.remoteplayer)
                {
                      if(MultiplayerManager.Players.TryGetValue(mess.Playerid,out RemotePlayer player))
                      {
            ...
            GUILayout.Space(10);
             ...
             foreach (SendReceiveIndex OutIndex in FileSyncing.OutGoingIndexes)
             {
                    ...
                    if (GUILayout.Button($"Cancel {OutIndex.TotalPacketsinFile - OutIndex.PacketNumbersStored.Count} packets of {OutIndex.TotalPacketsinFile} for {OutIndex.NameOfFile}"))
                        {
                        RipTideOutgoing.FileStatus(OutIndex.NameOfFile, (int)SyncStatus.Cancel);
            ...
            GUILayout.Space(10);
                ...
                foreach(SendReceiveIndex InIndex in FileSyncing.IncomingIndexes.ToArray())
                {
                    ...
                    if (InIndex.IsReceiving)
                    {
                        ...
                        {
                        RipTideOutgoing.FileStatus(InIndex.NameOfFile, (int)SyncStatus.Cancel);
        ...
        public void ShowBottomPanel()
        {
            ...
            GUILayout.Label($"Ping: {MultiplayerManager.Ping/1000} secs",MiniPanelStyle);
            GUILayout.Label($"Average delay: {MultiplayerManager.instance.GetAveragePing() /1000} secs", MiniPanelStyle);
            GUILayout.Label($"KB out /s: {MultiplayerManager.KBoutpersec} ", MiniPanelStyle);
            GUILayout.Label($"KB in /s: {MultiplayerManager.KBinpersec} ", MiniPanelStyle);
            GUILayout.Label($"Fragments: {MultiplayerManager.ActiveFragments} ", MiniPanelStyle);
            GUILayout.Label($"TransformDump: {MultiplayerManager.TransformDump} ", MiniPanelStyle);
            GUILayout.Label($"AudioDump: {MultiplayerManager.AudioDump} ", MiniPanelStyle);
            GUILayout.Label($"Pending Reliable: {MultiplayerManager.PendingReliable} ", MiniPanelStyle,GUILayout.MaxWidth(Screen.width / 9));
        ...
        public void ReceivedSpawnInvite(ushort from, Vector3 pos, Vector3 rot)
        {
            if(MultiplayerManager.Players.TryGetValue(from,out RemotePlayer player))
            {
              NewMessage(Constants.ServerMessageTime, new TextMessage($"{player.username} offered his spawn", FrostyUIColor.Player, TextMessage.Textmessagemode.remoteplayer));
                player.spawnpos = pos;
                player.spawnrot = rot;
        ...
        void ShowUpdateWindow()
        {
            ...
            if (Directory.Exists(MultiplayerManager.UpdateDir))
            {
                if(Directory.Exists(MultiplayerManager.UpdateDir + Versionofupdate + "/"))
                {
                    gotfiles = new DirectoryInfo(MultiplayerManager.UpdateDir + Versionofupdate + "/").GetFiles().Length;

                    if (new DirectoryInfo(MultiplayerManager.UpdateDir + Versionofupdate + "/").GetFiles().Length == UpdateFiles.Count)
                    {
            ...
            GUILayout.Space(50);
            GUILayout.Label($"Your Version of Frosty Manager: {RiptideManager.instance.VERSIONNUMBER}  doesnt match the Servers: {Versionofupdate}");
            GUILayout.BeginHorizontal();
        ...
        public void ShowPublicServers()
        {
            ...
            {
                StopCoroutine(MultiplayerManager.UpdatePublicServers());
                StartCoroutine(MultiplayerManager.UpdatePublicServers());
            }
            GUILayout.EndHorizontal();
            GUILayout.Space(20);
            ...
            {
                GUILayout.Label("Requesting connection..");
                if (GUILayout.Button("Cancel"))
                {
                    StopCoroutine(MultiplayerManager.RequestServerConnection(""));
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        publicserverscroll = GUILayout.BeginScrollView(publicserverscroll);
                        ...
                        {
                            GUILayout.Label("No public servers found");
                        }
        ...
        /// <summary>
        /// each received public server from hub is stored, if online menu is open, each server in list is sent here to be represented in public server column
        /// </summary>
        /// <param name="server"></param>
        void ShowPublicServerButton(PublicServer server)
        {
            GUILayout.Space(5);
            if(GUILayout.Button($"{server.Name} : {server.playercount} of {server.maxplayers} riding now"))
            {
                Showingserver = server;
                Publicserverdetailwindowopen = true;

            }
           
        }

        void DetailedServerWindowShow()
        {
            if(Showingserver != null)
            {
                GUI.skin = skin;
                GUILayout.BeginArea(new Rect(new Vector2(Screen.width/3,Screen.height/4), new Vector2(Screen.width / 3, Screen.height / 4f)), BoxStyle);
                GUILayout.Space(20);
                GUILayout.Label($"Server name: {Showingserver.Name}");
                GUILayout.Space(5);
                GUILayout.Label($"{Showingserver.playercount} riding now, {Showingserver.maxplayers} maximum");
                GUILayout.Space(5);
                GUILayout.Label($"most popular map right now: {Showingserver.mostpopmap}");
                GUILayout.Space(5);
                GUILayout.Label($"Tickrate: {Showingserver.tickrate} per second");
                GUILayout.Space(5);
                GUILayout.Label($"Server version: {Showingserver.version} - Your version {RiptideManager.instance.VERSIONNUMBER}");
                GUILayout.Space(10);
                if (GUILayout.Button("Connect"))
                {
                    NewMessage(Constants.SystemMessageTime, new TextMessage("Trying Setup..", FrostyUIColor.System, 0));
                    StartCoroutine(MultiplayerManager.RequestServerConnection(Showingserver.Name));
                    ...
                    try
                    {
                      ...
                      if (MultiplayerManager.PlayerSavedata != null)
                      {
                        MultiplayerManager.PlayerSavedata.Username = Username;
                        string json = TinyJson.JSONWriter.ToJson(MultiplayerManager.PlayerSavedata);
                        File.WriteAllText(MultiplayerManager.Playersavedir + "PlayerData.json", json);
                      }
                    ...
                    {
                        UnityEngine.Debug.Log($"Playerdata save error: {x}");
                    }
                    ...
                    if (LocalPlayer.RiderModelname == "Daryien")
                    {
                        ...
                        {
                          instance.NewMessage(Constants.SystemMessageTime, new TextMessage("Couldn't load Daryien Save, Select a texture for all Daryien parts and save for sync of custom Daryien", FrostyUIColor.Server, 0));
                        }
                        else
                        {

                          instance.NewMessage(Constants.SystemMessageTime, new TextMessage("Loaded Daryien's Save", FrostyUIColor.Server, 0));
                        }
                    ...
                    try
                    {
                        MultiplayerManager.instance.GetLevelName();
                    }
                    catch (Exception x)
                    {
                ...
                if (GUILayout.Button("close"))
                {
                    Publicserverdetailwindowopen = false;
                    Showingserver = null;
                }
        ...
        /// <summary>
        /// called by timer if online menu is open to refresh server list
        /// </summary>
        void UpdateServerList()
        {
            if (GethubDataWatch.Elapsed.TotalSeconds > 60 && MultiplayerManager.isConnected())
            {
                StartCoroutine(MultiplayerManager.UpdatePublicServers());
                GethubDataWatch.Reset();
                GethubDataWatch.Start();
            }
            else if (!GethubDataWatch.IsRunning)
            {
                GethubDataWatch.Reset();
                GethubDataWatch.Start();
            }
