...
namespace PIPE_Multiplayer
{
    public class InGameUI : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                {(int)PIPE_Multiplayer.MessageColourByNum.Me,Color.blue},
                {(int)PIPE_Multiplayer.MessageColourByNum.Player,Color.black},
                {(int)PIPE_Multiplayer.MessageColourByNum.System,Color.green},
                {(int)PIPE_Multiplayer.MessageColourByNum.Server,Color.red},
            };
       ...
       public void MiniGUI()
        ...
        public void ShowPublicServers()
        {
            ...
            if (RequestingConnection)
            {
                ...
                {
                    StopCoroutine(GameManager.RequestServerConnection(""));
        ...
        void DetailedServerWindowShow()
        {
            if(Showingserver != null)
            {
                ...
                GUILayout.Space(5);
                GUILayout.Label($"Server version: {Showingserver.version} - Your version {GameNetworking.instance.VERSIONNUMBER}");
                GUILayout.Space(10);
                if (GUILayout.Button("Connect"))
                {
                    NewMessage(Constants.SystemMessageTime, new TextMessage("Trying Setup..", 1, 0));
                    StartCoroutine(GameManager.RequestServerConnection(Showingserver.Name));
