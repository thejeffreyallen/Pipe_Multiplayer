...
namespace FrostyP_Game_Manager
{
	public class CameraSettings : MonoBehaviour
	{
		...
		public void Show()
		{
			...
			if(Camera.current.gameObject != PIPE_Multiplayer.CharacterModding.instance.Cam)
            {
			...
			GUILayout.BeginArea(new Rect(new Vector2(50, 100), new Vector2(Screen.width / 3, Screen.height - 150)), PIPE_Multiplayer.InGameUI.BoxStyle);
			...
			if (GUILayout.Button("Close Camera Setup"))
			{
                ...
                if (PIPE_Multiplayer.InGameUI.instance.IsSpectating)
                {
				PIPE_Multiplayer.InGameUI.instance.ShowCamSettingsOverride = false;
				}
                else
                {
			...
			GUILayout.Space(10);
			GUILayout.Label(lastpresetselectedCAM + " Profile active", PIPE_Multiplayer.InGameUI.instance.Generalstyle);
			GUILayout.Space(20);
			...
			GUILayout.Label("Camera FOV = " + CamFOVCAMSETTING , PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			CamFOVCAMSETTING = GUILayout.HorizontalSlider(CamFOVCAMSETTING, 0, 100);
			GUILayout.Label("Bloom Intensity = " + newbloomintvalueCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			newbloomintvalueCAMSETTING = GUILayout.HorizontalSlider(newbloomintvalueCAMSETTING, 0, 1f);
			GUILayout.Label("Bloom Threshold = " + newbloomvalueCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			newbloomvalueCAMSETTING = GUILayout.HorizontalSlider(newbloomvalueCAMSETTING, 0, 10);
			GUILayout.Label("Aperture (0-16) = " + apertureCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			apertureCAMSETTING = GUILayout.HorizontalSlider(apertureCAMSETTING, 0, 16);
			GUILayout.Label("Focus Distance (0-10) = " + focusdistanceCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			focusdistanceCAMSETTING = GUILayout.HorizontalSlider(focusdistanceCAMSETTING, 0, 10);
			GUILayout.Label("Contrast (0-75) = " + ContrastNewCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			ContrastNewCAMSETTING = GUILayout.HorizontalSlider(ContrastNewCAMSETTING, 0, 75);
			GUILayout.Label("Saturation (-100 - 50) = " + SaturationNewCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			SaturationNewCAMSETTING = GUILayout.HorizontalSlider(SaturationNewCAMSETTING, -100, 50);
			GUILayout.Label("Brightness (-50 - 50) = " + BrightnessNewCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			BrightnessNewCAMSETTING = GUILayout.HorizontalSlider(BrightnessNewCAMSETTING, -50, 50);
			GUILayout.Label("Temperature (-30 - 30) = " + temperatureNewCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			temperatureNewCAMSETTING = GUILayout.HorizontalSlider(temperatureNewCAMSETTING, -30, 30);

			GUILayout.Label("Vignette Intensity (0 - 1) = " + vignetteintensityCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			vignetteintensityCAMSETTING = GUILayout.HorizontalSlider(vignetteintensityCAMSETTING, 0, 1f);

			GUILayout.Label("Vignette Roundness (0 - 1) = " + VignetteRoundnessCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			VignetteRoundnessCAMSETTING = GUILayout.HorizontalSlider(VignetteRoundnessCAMSETTING, 0, 1);

			GUILayout.Label("Grain Intensity (0 - 1) = " + GrainIntensityCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			GrainIntensityCAMSETTING = GUILayout.HorizontalSlider(GrainIntensityCAMSETTING, 0, 1);

			GUILayout.Label("Grain Light contribution (0 - 1) = " + GrainLightcontribCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			GrainLightcontribCAMSETTING = GUILayout.HorizontalSlider(GrainLightcontribCAMSETTING, 0, 1);

			GUILayout.Label("Grain Size (0 - 1) = " + GrainSizeCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
			GrainSizeCAMSETTING = GUILayout.HorizontalSlider(GrainSizeCAMSETTING, 0, 1);

			GUILayout.Label("Hue Shift (-50 - 50) = " + HueShiftCAMSETTING, PIPE_Multiplayer.InGameUI.instance.MiniPanelStyle);
